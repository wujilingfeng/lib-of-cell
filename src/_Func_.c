...
//node->Prev=NULL;
        Node_init(node);
        node->Next=(void*)temp_v[i]->faces;
        if(temp_v[i]->faces!=NULL)
        {   
            temp_v[i]->faces->Prev=(void*)node;         
...
//node->Prev=NULL;
        Node_init(node);
        node->Next=(void*)temp_v[i]->cells;
        if(temp_v[i]->cells!=NULL)
        {
            temp_v[i]->cells->Prev=(void*)node;
        }
        node->value=(void*)c;
        temp_v[i]->cells=node;
...
//node2->Prev=NULL;
        Node_init(node2);
        node2->Next=(void*)c->halffaces;
        if(c->halffaces!=NULL)
        {
            c->halffaces->Prev=(void*)node2;
        }
        node2->value=(void*)hf;
        c->halffaces=node2;
//
...
if(f->halffaces[0]->cell!=NULL)
{
node1->value=(void*)f->halffaces[1];
}
else
{
node1->value=(void*)f->halffaces[0];
}
//node1->value=(void*)f;
node3=node1;
//node1->value=
...
{
//auto fit=temp_faces.find(((template_f*)node2->value)->id);
if(temp_faces[((template_f*)node2->value)->id]==0)
{
	//printf("found\r\n");
Node* node1=(Node*)malloc(sizeof(Node));
Node_init(node1);
...
node1=(Node*)malloc(sizeof(Node));
Node_init(node1);
node1->Next=(void*)node3;
node3->Prev=(void*)node1;
if(((template_f*)node2->value)->halffaces[0]->cell!=NULL)
{

node1->value=(void*)(((template_f*)node2->value)->halffaces[1]);
}
else
{
node1->value=(void*)(((template_f*)node2->value)->halffaces[0]);
}
//node1->value=node2->value;
node3=node1;
goto push;
...
template_hf* hf1,hf2;
own->external_cell.halffaces=own->node_of_boundary_face(own,f);
