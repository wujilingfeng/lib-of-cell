...
bool Mesh_vertex_is_boundary(struct Mesh* own,template_v &v)
{
    ...
    while(node!=NULL)
    {
        if(Mesh_face_is_boundary(own,(template_f*)node->value))
        {   return true;}
        node=(Node*)(node->Next);
    }
    return false;
...
Node* Mesh_node_of_boundary_face(struct Mesh* own,template_f *f_)
{
    ...
    if(!Mesh_face_is_boundary(own,f_))
    {return NULL;}
    else
    {
        //printf("shi\n");
...
#endif
    for(auto it=own->faces.begin();it!=own->faces.end();it++)
    {   if(Mesh_face_is_boundary(own,it->second))
        {
...
void Mesh_init(struct Mesh* own)
{
    ...
    own->face_is_boundary=Mesh_face_is_boundary;
