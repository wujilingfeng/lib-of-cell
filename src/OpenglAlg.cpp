...
void Mesh_view_data_init(Mesh_view_data *m)
{
   ...
   // m->F_V=NULL;
    ...
    int size=node_size(m->F_V),size1=node_size(m->E_V);
   ...
   glGenVertexArrays(size+size1+1,m->VAOs);
    if(m->Buffers!=NULL)
    {
        free(m->Buffers);
    }
...
/*    if(m->F_color!=NULL)
    ...
    m->Buffers=(GLuint**)malloc(sizeof(GLuint*)*(size+size1));
